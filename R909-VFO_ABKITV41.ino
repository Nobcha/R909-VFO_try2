// R909-VFO V4.1  2024.10.01
// H:\nob\AIRBAND RECEIVER\AB_kit\No.4\R909-VFO_ABKITV41\R909-VFO_ABKITV41.ino
// 
// R909-DSP2 radio V1  20240715
//  H:\nob\AIRBAND RECEIVER\9H5BM_5351\R909_sdr\R909-DSP2\RF_V2\R909-DSP2-V1.5
//
...
unsigned long int interfreq =  10700000; // 10.70Mhz  <<<<<<
...
// functions
  ...
  void Step_Disp(void);                 //  L#400,#1377,#1429
  void Freq_Disp(void);
...
volatile boolean display_ref = 0;
...
//  getSettings();
  ...
  if (freq1 < 10000000 || freq1 > 200000000) { //10-200MHz
    display.print(F("FREQ Err")) ;
    display.display();
    while ( digitalRead(RESW) == 1);  // Wait responce
    ...
    EEPROM.put( previous_freqFM_AD, 103900000 );    // 
    EEPROM.put( previous_freqAM_AD, 118100000 );    // 
  }
...
// Starting up Si5351a  >>>>>>>>>
  ...
  if (if_mode == true) {
    freq_0 = freq1  + interfreq  ;   // on IF mode xx.xMHz upper
  }
...
// Basic function for timer control OLED blink
  blink(300, 500);              // Blinking for FUNC display on OLED
...
// process depended on mode
  switch ( mode ) {
    ...
    case FUNCTION: {                     // function mode is 12 
      ...
      display_ref = 1;                   // Display activating
      break;
...
//          set21400kHz();              // Change to AM 21.4MHz mode
    ...
    case FREQSET: {                      //  FREQ button long push 
        ...
        mode = FREQ;                          // Back to FREQ mode
        Timer_Mem_Write=400;                  // Until FREQ displaying, clear 
        display.clearDisplay(); 
        Mem_Write1=1;     
...
//          }
  ...
  if (( millis() - elapsedRSSI ) > Time_elapsedRSSI ){  // 300ms
    s_meter_Disp();
    elapsedRSSI = millis();
  // Squelch rssi=ADC > s_value
    if(SquelchLevel>s_value){
...
//      digitalWrite (SQLMUTE, LOW);        // Activate the amp
  ...
  if (func_sw_value < 900) {
    ...
    switch ( sw_stat){
      case 1:
        mode = MEMORY;           // freq mode 0
        break; 
      case 2:   
        mode = SCAN;         // VOLUME mode 5
        break;      
    }
  }
...
// After memory written, flushing display
  if(Timer_Mem_Write>0 && Mem_Write1==1 ){       
    sprintf(eachdisp, spdisp) ;    // 2nd line 1st character step display
    display_ref = 1;               // Display refresh
  }
  if(Timer_Mem_Write<=0){
    if ( mode == ( FREQSET | STEPPUT | MEMORYPUT )) {
      mode = FREQ ;
      display_ref = 1;
      Mem_Write1=0;      
    }
  ...
  if ( display_ref ==1 ){              // Display refresh
    display.display();
    display_ref = 0;
  }
...
void TimerCount(void) {
  ...
  if (Timer_LED > 0) Timer_LED = Timer_LED - time_dif;    // To turn on/off for #13 LED & OLED
...
void FUNC_Disp ( char* func){        // Display FUNC of #2 size on X:0,Y:25 
  ...
  display_ref = 1;                   // activating display 
}
...
// Serial.ino   too much memory needed
...
// fcontext = 0 > CLK0, fcontext = 1 > CLK1
void send_frequency(int32_t ffrequency, int fcontext) {
  switch (fcontext) {        // no use fcontext  only CLK0
    case 0:                                     // Port 1
      ...
      break;
    case 1:                                     // Port 2
      if ( ffrequency != last_ffrequency1 ){      // no use fcontext  only CLK0
        set_freq((uint64_t)ffrequency );
        last_ffrequency1 = ffrequency;
      }
      break;
    case 2:                                     // Port 3
      if ( ffrequency != last_ffrequency2 ){      // no use fcontext  only CLK0
        set_freq((uint64_t)ffrequency );
...
//  snr = rx.getCurrentSNR();
  ...
  rssi = analogRead( AGC_port );        //   >>>>>>>>>>>>>>> ADC 0.8->4V
                                        //  164-819
  s_value = map(rssi, 165, 820, 0, 9);     // s max changed
...
void drawbargraph() {
  ...
  display.print(F("SM"));  
  if(s_value > 12) s_value = 12;    // To protect over value
...
// To display step frequency  
  ...
  if (fstep_idx == 1) display.print(F("8.3kHz")); 
  ...
  if (mode == FREQ) {                    // FREQ:mode 0 : Up/Down frequency
    if (dir == 0x10){                    // Clockwise
      ...
      if (BandSelect == true) {          // AM mode limit
        if (freq1 > (long) 160000000) {  //  140MHz -> 100MHz
          freq1 = (long)   160000000;
        }
      }
      freq_0 = freq1;                // Written value for 5351a
      if (if_mode == true) {
